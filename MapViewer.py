...
class MapViewerPlane(ScatterPlane):
  ...
  '''
    ...
    def tile_exist(
        ...
        return self.tileserver.exist(nx, bound - ny - 1, zoom,
                self.maptype)
    ...
    def tile_draw(
        ...
        image = self.tileserver.get(nx, bound - ny - 1, zoom,
                                    self.maptype)
        ...
        with self.canvas:
            Color(1, 1, 1, alpha)
            Rectangle(pos=(tx, ty), size=(sx, sy),
    ...
    # check if we must invalidate the tiles
        ...
        if not self.lastmove is 0 and time.time() > self.lastmove \
            + INACTIVITY_TIMEOUT:
            _exit(1)
        ...
        for overlay in self.overlays:
            if overlay.type == 'wms':
                ...
                if self.lastmove is None or time.time() > self.lastmove \
                    + 0.5:  # wait a second after moving before we try to contact the WMS
                    image = overlay.get(self, parent.width,
                            parent.height)
                ...
                if image not in (None, False) and image.loaded:
                    ...
                    alpha = max(0, min((time.time()
                                - self.loadtimes[image]) * 1,
                                overlay.max_alpha))  # fadein
                    oldalpha = overlay.max_alpha - alpha
                    if oldalpha == 0:  # as soon as the old image is faded out, put this one in the cache
                        self.overlaycache[overlay.provider_name
                                + overlay.layer] = (image, self.cmin,
                                self.csize)  # self.omin, osize
                    with self.canvas:
                        Color(1, 1, 1, alpha)
                        Rectangle(pos=self.cmin, size=self.csize,
                                  texture=image.texture)

          # try displaying the previous image from this overlay, until the next one is fully displayed

                (image, pos, isize) = \
                    self.overlaycache.get(overlay.provider_name
                        + overlay.layer, (None, None, None))
                if image:
                    with self.canvas:
                        Color(1, 1, 1, oldalpha)
                        Rectangle(pos=pos, size=isize,
                                  texture=image.texture)
            ...
            # display the legend graphic
                ...
                if self.lastmove is None or time.time() > self.lastmove \
                    ...
                    geometries = overlay.get(self, parent.width,
                            parent.height)
        ...
        for overlay in self.overlays:
            ...
            if tip is not None:
                popup = Popup(title='WFS FeatureInfo',
                              content=Label(text=tip), size_hint=(None,
                              None), size=(800, 400))
                popup.open()
                break
    ...
    def apply_angle_scale_trans(
        ...
        if newscale < self.minscale or newscale > self.maxscale:
            newscale = 1
        tf = Matrix().rotate(angle, 1, 1, 1).scale(scale, scale,
...
class MapViewer(StencilView):
    ...
    def on_touch_down(self, touch):
        if self.readonly:  # map is not to be clicked
            return
    ...
    def on_touch_move(self, touch):
        if self.readonly:  # map is not to be scrolled/zoomed
            return
        if not self.collide_point(*touch.pos):  # touch is not within bounds
            return False
